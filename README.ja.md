...
曜日周期なら `period=7`、月次データなら `period=12`、その他どんな周期長でも指定
...
四半期（周期4）専用にハードコードされている。日次データの週周期、週次データの
年周期など、多くの実データはその型に収まらない。

本パッケージはX-11の手順を分解する：外れ値調整・休日調整・水準シフト調整・予測
延長といった、本質的に周期に依存しない部分はX-13ARIMA-SEATSのRegARIMA事前調整
に任せる（すでに十分な実績がある）。周期に縛られるX-11コア部分——移動平均・
季節フィルターの自動選択・極端値の置換——のみを本パッケージで再実装し、任意の
周期長に一般化する。
...
- 極端SI比の置換（X-11 extreme value replacement）
...
| `replace_extreme` | `True` | 極端SI比の置換（X-11 extreme value replacement） |
| `sigma` | `(1.5, 2.5)` | 極端SI比置換の（下限、上限）シグマ |
...
AGPLの条件で利用できない場合（プロプライエタリ製品への組込み等）は商用ライ
センスを用意している。共同研究・カスタマイズの相談も歓迎する。連絡は
GitHubのIssueを開いて行ってほしい。
